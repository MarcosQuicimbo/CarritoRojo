...
buttons = {'x':0,'o':0,'t':0,'s':0,
           'L1':0,'R1':0,'L2':0,'R2':0,
           'share':0,'options':0,
           'axis1':0.,'axis2':0.,'axis3':0.,'axis4':0.}#float, -1,1,4axis, up,down,left,right
axiss=[0.,0.,0.,0.,0.,0.]#yo creo que aqui estan de mas 2-> 0's
...
def getJS(name=''):
    ...
    global buttons
    # retrieve any events ...
    for event in pygame.event.get():  #ask for the event            # Analog Sticks
        if event.type == pygame.JOYAXISMOTION:
            axiss[event.axis] = round(event.value,2)
        elif event.type == pygame.JOYBUTTONDOWN:                    # When button pressed botones en general
            #print(event.dict, event.joy, event.button, 'PRESSED')
            for x,(key,val) in enumerate(buttons.items()):
                if x<10:
                    if controller.get_button(x):buttons[key]=1 #pongo 1 si esta siendo presionado, 0 liberado
